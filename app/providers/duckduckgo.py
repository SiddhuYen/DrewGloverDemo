"""DuckDuckGo HTML provider — free fallback for locating pages to scrape.

Used when Serper is unconfigured/exhausted. Protected by a token bucket,
jittered spacing, and a circuit breaker that trips on repeated 429s. Like
Serper, it only finds URLs; it never asserts a relationship.
"""
...
class DuckDuckGoProvider(SearchProvider):
    ...
    def __init__(self) -> None:
        self._bucket = TokenBucket(
            rate_per_sec=1.0 / max(config.DDG_MIN_INTERVAL, 0.01),
        ...
        for res in soup.select("div.result, div.web-result"):
            ...
            if url and title:
                out.append(SearchResult(
                    title, url, snip.get_text(" ", strip=True) if snip else "",
                    self.name))
        if not out:  # lite.duckduckgo.com markup
