"""Persistent SQLite-backed cache for search / page / structured responses.

Keyed by (provider, kind, ident). Values are JSON, TTL-checked on read. Lives in
a separate sqlite file from the graph DB and survives across runs. Thread-safe.

Contract: before any network request, callers check the cache; on a hit they
...
def get(key: str, track: bool = True):
    """Cached JSON value (counting a hit), or None (counting a miss).
    track=False for internal lookups that shouldn't skew provider stats."""
    now = time.time()
    with _lock:
        conn = _connect()
        row = conn.execute(
...
# --- persistent counters (Serper monthly quota; survives across runs) -------
...
def incr_counter(name: str, by: int = 1, ttl: int = 366 * 86400) -> int:
    """Atomically increment a persisted integer counter, returning the new value."""
    with _lock:
        conn = _connect()
        row = conn.execute("SELECT value FROM cache WHERE key = ?", (name,)).fetchone()
        n = 0
